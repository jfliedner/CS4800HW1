...
class InsertSort {
  ...
  static void insertionSort(ArrayList<Request> myRequests) {
    for (int j = 1; j < myRequests.size(); j++) {
      Request key = myRequests.get(j);
      int i = j - 1;
      while (i >= 0 && myRequests.get(i).compareTo(key) == 1) {
        myRequests.set(i + 1, myRequests.get(i));
        i--;
       }
      myRequests.set(i + 1, key);
    }
